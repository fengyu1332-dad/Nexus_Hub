...
import { Loader2, RefreshCw, Send, Mail, Users, FileText, Settings } from 'lucide-react'
...
export function NewsletterManager() {
  ...
  const handleSendTest = async () => {
    ...
    try {
      ...
      setSendResult(`发送完成 — 成功 ${data.sent} 封，失败 ${data.failed} 封，订阅者 ${data.totalSubscribers} 人，帖子 ${data.postsIncluded} 篇`)
    } catch (err: any) {
        ...
        <div className='space-y-4'>
          <div className='bg-white rounded-lg border p-6'>
            ...
            <div className='space-y-4'>
              <div className='p-4 bg-amber-50 rounded-lg border border-amber-200'>
                <p className='text-sm text-amber-800 font-medium mb-1'>⚠️ n8n 工作流状态</p>
              ...
              <div className='p-4 bg-zinc-50 rounded-lg border border-zinc-200'>
                <p className='text-sm text-zinc-700 font-medium mb-2'>手动发送测试</p>
                <p className='text-xs text-zinc-500 mb-3'>
                  向所有活跃订阅者发送本周 Newsletter。请确认 n8n 工作流已正确配置后再使用。
                </p>
                <button
          ...
          <div className='bg-white rounded-lg border p-6'>
            ...
            <ul className='text-xs text-zinc-500 space-y-2 list-disc list-inside'>
              <li>n8n 工作流需在 n8n 面板中激活 (<code className='bg-zinc-100 px-1 rounded'>nexus-architect-newsletter</code>)</li>
              <li>需要配置 <code className='bg-zinc-100 px-1 rounded'>NEWSLETTER_FROM</code> 环境变量为发件人邮箱</li>
              <li>需要配置 Resend API key 环境变量用于邮件发送</li>
              <li>需修改 n8n 工作流中的收件人为动态查询订阅者列表</li>
            </ul>
          </div>
        </div>
      )}
